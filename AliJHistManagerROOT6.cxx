#include "AliJHistManagerROOT6.h"
#include <TMath.h>
#include <TFile.h>
...
ClassImp(AliJNamed);
ClassImp(AliJBin);
ClassImp(AliJArrayBase);
ClassImp(AliJArrayAlgorithm);
ClassImp(AliJArrayAlgorithmSimple);
ClassImp(AliJTH1);
ClassImp(AliJTH1Derived<TH1D>);
ClassImp(AliJTH1Derived<TH2D>);
ClassImp(AliJTH1Derived<TProfile>);
ClassImp(AliJTH1DerivedPlayer<TH1D>);
ClassImp(AliJTH1DerivedPlayer<TH2D>);
ClassImp(AliJTH1DerivedPlayer<TProfile>);
ClassImp(AliJHistManager);
...
AliJNamed::AliJNamed(const AliJNamed& obj) :
  fName( obj.fName ),
  fTitle(obj.fTitle),
  fOption(obj.fOption),
  fMode(obj.fMode)
...
//_____________________________________________________
AliJNamed& AliJNamed::operator=(const AliJNamed& obj)
{
  // assignment operator
  if(this != &obj){
    fName =  obj.fName ;
...
TString AliJBin::GetString(){
  ...
  cout << "AliJBin\t"+fName+"\t"+fIndexName+"\t\""+fTitle+"\""+"\t"+fOption+"\t"+Join(fBinStr," ") << endl;
...
//_____________________________________________________
// vargyas
std::vector<double> AliJBin::GetBinVector(){
    return fBinD;
}
...
void * AliJTH1::BuildItem(){
  ...
  if( IsLoadMode() ){
    ...
    if( !item ){
      ...
      TH1 * item2 = NULL;
      while( Next(tmp) ){
        item2 = (TH1*)fSubDirectory->Get(BuildName());
        if(item2 ) break;
      }
      if( item2 ) {
        item = dynamic_cast<TH1*>((static_cast<TH1*>(item2))->Clone(name));
        item->Reset();
...
void AliJHistManager::WriteConfigToNew(TFile * newFile){
  newFile->cd();
  TDirectory * owd = gDirectory;
  owd->pwd();
  TDirectory * fHistConfigDir = owd->mkdir("HistManager");
  fHistConfigDir->cd();
  fHistConfigDir->pwd();
  cout<< fHistConfigDir <<endl; 
  fHistConfigDir->Print();

  // until that it is fine;
...
int AliJHistManager::LoadConfig( TString name ){
  TDirectory * dir = dynamic_cast<TDirectory*>(gDirectory->Get(name));
  if( !dir ){
    cout<<"JWARN :  no directory "<<name<<endl;
    return -1;
  }
  SetDirectory(  dir );
  return LoadConfig();
...
/*
int ReWriteRoot(TString name, TString outname ){
  TFile *fin = new TFile(name,"READ");
  if( !fin ) {
    cout<<"JERROR : No File - "<<name<<endl;
    return 1;
  }

  TList * list = gDirectory->GetList();
  if( !list ){
    cout<<"JERROR : No Object Under - "<<gDirectory->GetName()<<endl;
    return 2;
  }
