...
class AliJNamed : public TObject{
    protected:
        TString  fName;
        TString  fTitle;
        TString  fOption;
    ...
    public:
        ...
        virtual ~AliJNamed();
        AliJNamed(const AliJNamed& obj);
        AliJNamed& operator=(const AliJNamed& obj);
...
class AliJBin : public AliJNamed {
    ...
    public:
        ...
        int GetBin( double x );
        std::vector<double> GetBinVector(); //vargyas
...
class AliJArrayAlgorithmSimple : public AliJArrayAlgorithm {
    ...
    public:
        ...
        ClassDef(AliJArrayAlgorithmSimple,1);
};
...
class AliJTH1DerivedPlayer {
    ...
    public:
        ...
        T* Val(){ return static_cast<T*>(fCMD->GetItem()); }
        ...
        ClassDef(AliJTH1DerivedPlayer,1);
};
...
class AliJHistManager: public AliJNamed{
    ...
    public:
        ...
        TString GetString(){
            ...
            for( unsigned long i=0;i<fBin.size();i++ ){
                if( ! fBin[i] ) continue;
                st+=fBin[i]->GetString()+"\n";
            }
            //for( int i=0;i<GetNHist();i++ ){
            for( unsigned long i=0;i<fHist.size();i++ ){
                if( ! fHist[i] ) continue;
        ...
        void Write();
        void WriteConfig();
        void WriteConfigToNew(TFile * newFile);
